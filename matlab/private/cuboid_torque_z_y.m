...
sumx(a) = sumx(a)+((-1)^(i+j+k+l+m+n)).*((1/24).*(3.*Cv.*(w(a).^2).*...
    log(w(a).^2)-2.*(3.*Cv.*(w(a).^2).*log(-sqrt(w(a).^2))+3.*Cw.*...
    w(a).*(w(a)+2.*sqrt(w(a).^2))+4.*(w(a).^2).^(3/2))));
sumy(a) = sumy(a)+((-1)^(i+j+k+l+m+n)).*((-1/8).*w(a).*(Cu.*w(a).*...
    log(w(a).^2)-2.*(pi.*sqrt(w(a).^2).*Cw+Cu.*w(a).*...
    log(-sqrt(w(a).^2)))));
...
sumx(b) = sumx(b)+((-1)^(i+j+k+l+m+n)).*((1/12).*v(b).*(-3.*Cw.*v(b)+...
    6.*Cw.*v(b).*log(-sqrt(v(b).^2))+2.*sqrt(v(b).^2).*(3.*Cv+...
    v(b))));
...
sumx(c) = sumx(c)+((-1)^(i+j+k+l+m+n)).*((1/24).*(-4.*u(c).^3-6-6.*...
    ...
    Cv.*log(-sqrt(u(c).^2)).*u(c).^2+4.*(u(c).^2).^(3/2)));
...
sumx(d) = sumx(d)+((-1)^(i+j+k+l+m+n)).*((1/12).*(2.*r(d).*u(d).^2+3.*...
    u(d).*(v(d).^2+w(d).^2)-12.*u(d).*v(d).*(Cv+v(d)).*...
    acoth(u(d)./r(d))+2.*r(d).*(v(d).^2+3.*Cv.*v(d)-2.*w(d).^2)-...
    6.*u(d).*(v(d).^2+w(d).*(2.*Cw+w(d))).*log(r(d)-u(d))-3.*Cw.*...
    (u(d).^2-4.*u(d).*w(d)+v(d).^2+w(d).*(w(d)+2.*r(d)))-2.*u(d).^3-6.*...
    (Cv+v(d)).*(u(d).^2-w(d).^2).*acoth(v(d)./r(d))-12.*(Cw.*...
    v(d).^2.*acoth(w(d)./r(d))+u(d).*(Cw.*v(d).*atan(w(d)./r(d))...
    -(Cv+v(d)).*w(d).*atan((u(d).*v(d))./(w(d).*r(d)))+Cw.*v(d).*...
    atan((u(d).*w(d))./(v(d).*r(d)))))+6.*Cw.*(u(d).^2+v(d).^2).*...
    log(w(d)+r(d))));
sumy(d) = sumy(d)+((-1)^(i+j+k+l+m+n)).*((1/72).*(3.*(v(d).^3).*(2.*...
    log(v(d).^2+w(d).^2)+Cw.*atan((u(d).*w(d))./(v(d).*r(d))))+36.*...
    u(d).*v(d).*(2.*Cu+u(d)).*acoth(u(d)./r(d))-36.*Cu.*r(d).*...
    v(d)-9.*v(d).*(8.*Cw.*u(d).*(log(w(d)+r(d)))+w(d).^2+2.*w(d).*...
    (4.*Cw+w(d)).*log(u(d)+r(d))+u(d).^2-8.*Cw.*u(d))-30.*u(d).*...
    r(d).*v(d)+36.*((u(d).^2).*(Cu+u(d))-Cu.*(w(d).^2)).*...
    acoth(v(d)./r(d))+2.*(12.*(w(d).^2).*(3.*Cw+w(d)).*...
    atan(u(d)./w(d))+18.*Cw.*(u(d).^2+w(d).^2).*atan(w(d)./u(d))+...
    6.*w(d).*(w(d).^2+3.*Cw.*w(d)-3.*u(d).*(2.*Cu+u(d))).*...
    atan((u(d).*v(d))./(w(d).*r(d)))+u(d).*(3.*(u(d).^2).*...
