...
sumx(d) = sumx(d)+((-1)^(i+j+k+l+m+n)).*((1/12).*(2.*r(d).*u(d).^2+3.*...
    ...
    real(acoth(u(d)./r(d)))+2.*r(d).*(v(d).^2+3.*Cv.*v(d)-2.*w(d).^2)-...
    6.*u(d).*(v(d).^2+w(d).*(2.*Cw+w(d))).*log(r(d)-u(d))-3.*Cw.*...
    (u(d).^2-4.*u(d).*w(d)+v(d).^2+w(d).*(w(d)+2.*r(d)))-2.*u(d).^3-6.*...
    (Cv+v(d)).*(u(d).^2-w(d).^2).*real(acoth(v(d)./r(d)))-12.*(Cw.*...
    v(d).^2.*real(acoth(w(d)./r(d)))+u(d).*(Cw.*v(d).*atan(w(d)./r(d))...
