      ...
      for ll=[0,1]
        for mm=[0,1]
          for nn=[0,1]
            ...
            if any(a)
              Ex(a) = 1/8*w(a).*(-w(a).^2-2*Cw.*w(a)-8*Cv.*abs(w(a))+2*w(a).*(2*Cw+w(a)).*log(abs(w(a))));
              Ey(a) = 1/8*w(a).*(+w(a).^2+2*Cw.*w(a)+8*Cu.*abs(w(a))-2*w(a).*(2*Cw+w(a)).*log(abs(w(a))));
              Ez(a) = 1/4*(Cu-Cv)*w(a).^2.*log(abs(w(a)));
            end
            
            if any(b)
              Ex(b) = -1/4*Cw*v(b).*(v(b)+2*abs(v(b)));
              Ey(b) = -1/4*Cw*v(b).^2.*(log(v(b).^2)-1);
              Ez(b) = 1/72*v(b).*(2*v(b).^2+36*Cu*abs(v(b))+9*v(b).*(2*Cv+v(b)).*log(v(b).^2));
            end
            
            if any(c)
              Ex(c) = -1/4*Cw*u(c).^2.*(log(u(c).^2)-1);
              Ey(c) = -1/4*Cw*u(c).*(u(c)+2*abs(u(c)));
              Ez(c) = 1/72*u(c).*(2*u(c).^2+36*Cv*abs(u(c))+9*u(c).*(2*Cu+u(c)).*log(u(c).^2));
            end
            
            if any(d)
              
              Ex(d) = (1/8).*(...
                Cw.*(8.*v(d).*u(d)-2*s(d).^2-4.*v(d).*s(d))-...
                w(d).*(-8.*v(d).*u(d)+s(d).^2+8.*Cv.*s(d)+6.*v(d).*s(d))+...
                2.*(2.*Cw+w(d)).*(u(d).^2+w(d).^2).*log(v(d)+s(d))+...
                4.*(...
                2.*Cv.*u(d).*w(d).*acoth(u(d)./s(d)) + ...
                w(d).*(v(d).^2+2.*Cv.*v(d)-w(d).*(2.*Cw+w(d))).*acoth(v(d)./s(d)) - ...
                u(d).*(...
                2*w(d).*(Cw+w(d)).*atan(v(d)./w(d)) + ...
                2*v(d).*(Cw+w(d)).*log(s(d)-u(d)) + ...
                (w(d).^2+2.*Cw.*w(d)-v(d).*(2.*Cv+v(d))).*atan( u(d).*v(d)./(w(d).*s(d)) ) ...
                )...
                )...
                );
              ...
              Ey(d) = (1/8)*...
                ...
                4.*w(d).*(w(d).^2+2.*Cw.*w(d)-u(d).*(2.*Cu+u(d))).*acoth(u(d)./s(d))+...
                4.*v(d).*(-2.*Cu.*w(d).*acoth(v(d)./s(d))+2.*w(d).*(Cw+w(d)).*atan(u(d)./w(d))...
