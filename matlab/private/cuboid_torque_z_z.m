      ...
      for ll=[0,1]
        for mm=[0,1]
          for nn=[0,1]
            ...
            if any(a)
%              error a
              Ex(a) = 1/8*w(a).*(-w2(a)-2*Cw(a).*w(a)-8*Cv(a).*abs(w(a))+w(a).*(2*Cw(a)+w(a)).*log(w2(a)));
              Ey(a) = 1/8*w(a).*(+w2(a)+2*Cw(a).*w(a)+8*Cu(a).*abs(w(a))-w(a).*(2*Cw(a)+w(a)).*log(w2(a)));
            ...
            if any(d)
              ...
              Ex(d) = 1/8.*(...
                - s2(d).*(2.*Cw(d)+w(d)) + ...
                - 2.*s(d).*( 2.*Cw(d).*v(d) + 4.*w(d).*Cv(d) + 3.*w(d).*v(d) ) + ...
                + 8.*Cv(d).*u(d).*w(d).*acoth(s(d)./u(d)) + ...
                - 8.*u(d).*v(d).*(Cw(d)+w(d)).*log(s(d)-u(d)) + ...
                + 2.*(2.*Cw(d)+w(d)).*(u2(d)+w2(d)).*log(v(d)+s(d)) + ...
                + 4.*w(d).*(v2(d)-w2(d)+2.*Cv(d).*v(d)-2.*w(d).*Cw(d)).*acoth(s(d)./v(d)) + ...
                - 4.*u(d).*(w2(d)-v2(d)+2.*Cw(d).*w(d)-2.*v(d).*Cv(d)).*atan(u(d).*v(d)./(w(d).*s(d))) ...
                           );
              
              Ey(d) = (1/8)*...
                ((2.*Cw(d)+w(d)).*u(d).^2-8.*u(d).*v(d).*(Cw(d)+w(d))+8.*u(d).*v(d).*(Cw(d)+w(d)).*log(s(d)-v(d))...
                +4.*Cw(d).*u(d).*s(d)+6.*w(d).*s(d).*u(d)+(2.*Cw(d)+w(d)).*(v(d).^2+w(d).^2)+...
                4.*w(d).*(w(d).^2+2.*Cw(d).*w(d)-u(d).*(2.*Cu(d)+u(d))).*acoth(s(d)./u(d))+...
                4.*v(d).*(-2.*Cu(d).*w(d).*acoth(s(d)./v(d))+2.*w(d).*(Cw(d)+w(d)).*atan(u(d)./w(d))...
                +(w(d).^2+2.*Cw(d).*w(d)-u(d).*(2.*Cu(d)+u(d))).*atan(u(d).*v(d)./(w(d).*s(d))))...
                -2.*(2.*Cw(d)+w(d)).*(v(d).^2+w(d).^2).*log(u(d)+s(d))+8.*Cu(d).*w(d).*s(d));
              
              Ez(d) = (1/36).*(-u(d).^3-18.*v(d).*u(d).^2-6.*u(d).*(w(d).^2+6.*Cu(d)...
                .*v(d)-3.*v(d).*(2.*Cv(d)+v(d))+3.*Cv(d).*s(d))+v(d).*(v(d).^2+6.*(w(d).^2+...
                3.*Cu(d).*s(d)))+6.*w(d).*(w(d).^2-3.*v(d).*(2.*Cv(d)+v(d))).*atan(u(d)./w(d))...
                -6.*w(d).*(w(d).^2-3.*u(d).*(2.*Cu(d)+u(d))).*atan(v(d)./w(d))-9.*...
                (2.*(v(d).^2+2.*Cv(d).*v(d)-u(d).*(2.*Cu(d)+u(d))).*w(d).*atan(u(d).*v(d)./(w(d).*s(d)))...
                -2.*u(d).*(2.*Cu(d)+u(d)).*v(d).*log(s(d)-u(d))-(2.*Cv(d)+v(d)).*(v(d).^2-w(d).^2)...
                .*log(u(d)+s(d))+2.*u(d).*v(d).*(2.*Cv(d)+v(d)).*log(s(d)-v(d))+(2.*Cu(d)+...
