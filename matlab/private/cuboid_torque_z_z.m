...
%              error a
            ...
            if any(d)
              ...
              Ey(d) = 1/8*(...
                + (2.*Cw(d)+w(d)).*u2(d) + ...
                - 8.*u(d).*v(d).*(Cw(d)+w(d)) + ...
                + 8.*u(d).*v(d).*(Cw(d)+w(d)).*log(s(d)-v(d)) + ...
                + 4.*Cw(d).*u(d).*s(d) + ...
                + 6.*w(d).*s(d).*u(d) + ...
                + (2.*Cw(d)+w(d)).*(v2(d)+w2(d)) + ...
                + 4.*w(d).*(w2(d)+2.*Cw(d).*w(d)-u(d).*(2.*Cu(d)+u(d))).*acoth(s(d)./u(d)) + ...
                + 4.*v(d).*(...
                 - 2.*Cu(d).*w(d).*acoth(s(d)./v(d)) + ...
                 + 2.*w(d).*(Cw(d)+w(d)).*atan(u(d)./w(d)) + ...
                 + (w2(d)+2.*Cw(d).*w(d)-u(d).*(2.*Cu(d)+u(d))).*atan(u(d).*v(d)./(w(d).*s(d))) ...
                           ) + ...
                - 2.*(2.*Cw(d)+w(d)).*(v2(d)+w2(d)).*log(u(d)+s(d))+8.*Cu(d).*w(d).*s(d) ...
                         );
              
              Ez(d) = 1/36.*(...
                - u(d).^3 + ...
                - 18.*v(d).*u2(d) + ...
                - 6.*u(d).*(w2(d)+6.*Cu(d).*v(d)-3.*v(d).*(2.*Cv(d)+v(d))+3.*Cv(d).*s(d)) + ...
                + v(d).*(v2(d)+6.*(w2(d)+3.*Cu(d).*s(d))) + ...
                + 6.*w(d).*(w2(d)-3.*v(d).*(2.*Cv(d)+v(d))).*atan(u(d)./w(d)) + ...
                - 6.*w(d).*(w2(d)-3.*u(d).*(2.*Cu(d)+u(d))).*atan(v(d)./w(d)) + ...
                - 9.*( ...
                  + 2.*(v2(d)+2.*Cv(d).*v(d)-u(d).*(2.*Cu(d)+u(d))).*w(d).*atan(u(d).*v(d)./(w(d).*s(d))) + ...
                  - 2.*u(d).*v(d).*(2.*Cu(d)+u(d)).*log(s(d)-u(d)) + ...
                  - (2.*Cv(d)+v(d)).*(v2(d)-w2(d)).*log(s(d)+u(d)) + ...
                  + 2.*u(d).*v(d).*(2.*Cv(d)+v(d)).*log(s(d)-v(d)) + ...
                  + (2.*Cu(d)+u(d)).*(u2(d)-w2(d)).*log(s(d)+v(d)) ...
                     ) ...
                            );
